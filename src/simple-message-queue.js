            ...
            smq.bypassTimer = setTimeout(function bypassSend() {
                ...
                if (m) {
                    ...
                    node.status({
                        fill: "green",
                        shape: "ring",
                        text: "".concat(context.queue.length),
                    });
                }
                else {
                    smq.bypassTimer = null;
    ...
    function SimpleMessageQueueNode(config) {
        ...
        this.on("input", function (msg, nodeSend, nodeDone) {
            ...
            if (msg.reset !== undefined) {
                ...
                var last_elem = context.backup_queue.shift();
                if (last_elem) {
                ...
                var last_elem = context.queue.shift();
                if (last_elem) {
                    if (smq.backupMessages)
                        context.backup_queue.push(last_elem);
                ...
                if (context.is_disabled || (smq.firstMessageBypass && !smq.isBusy)) {
                    ...
                    if (context.capacity <= context.queue.length &&
                        context.capacity > 0) {
                        // 0 means no limit
                        node.status({
                            fill: "yellow",
